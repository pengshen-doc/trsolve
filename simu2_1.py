...
def greedy_path_on_nodes(phi, efrom, eto, src, dst=None, goal_set=None, coords=None, max_steps=500000):
    ...
    while not reached_goal(cur) and steps < max_steps:
        ...
        if lower:
            ...
            print("equal phi neighbor", steps)
            # Plateau creep: move to equal-phi neighbor that gets closer to goal
            equal = [v for v in nlist if abs(phi[v] - phi[cur]) <= tol]
            if not equal:
                break
            # choose the equal-phi neighbor by sorting distance to goal
            # pick the second-closest when available, otherwise pick the closest
...
def case_hex_with_jam_offsets(save_prefix="jam_hex", show=False, jam_factor=jamfactor,offsets=[0.0]):
    """
