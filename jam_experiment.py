...
JAM_FACTOR   = 50.0   # slow edges in disk: K^I -> K^I / JAM_FACTOR (i.e., time × JAM_FACTOR)
...
JAM_OFFSET_FACTORS = [0.0, 1/3, 2/3, 1.0, 2.0]  # offsets for f style sweep (Δy = factor × radius)
...
def _edges_intersecting_disk(coords, efrom, eto, center_xy, radius_ft):
    ...
    for u, v in zip(efrom, eto):
        ...
        # slow if both endpoints are inside, or segment intersects the disk boundary
        inside_endpoints = (
            (np.linalg.norm(pu - center_xy) <= radius_ft) and
            (np.linalg.norm(pv - center_xy) <= radius_ft)
        )
        seg = LineString([tuple(pu), tuple(pv)])
        if inside_endpoints or pdisk.intersects(seg):
            slow.add((a, b))
    return slow
