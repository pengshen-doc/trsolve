...
def case_hex_with_jam_offsets(save_prefix="jam_hex", show=False, jam_factor=10000.0,
                              offsets=(0.0, 1/3, 2/3, 1.0, 2.0)):
    """
    ...
    for i, off in enumerate(offsets, start=1):
        ...
        # Remove all edges that intersect the jam disk
        keep = ~slow_mask_arr
        efrom_j = np.array(efrom)[keep]
        eto_j   = np.array(eto)[keep]
        etime_jam = np.array(etime)[keep].copy()

        rows, cols, data = [], [], []
        time_map_jam = {}
        for (u, v, t0) in zip(efrom_j, eto_j, etime_jam):
            t = t0
            G = 1.0 / max(t, 1e-12)
        ...
        midpts = 0.5 * (coords[efrom_j] + coords[eto_j])
        conductance = np.array([-M_jam[u, v] for u, v in zip(efrom_j, eto_j)], dtype=float)
        ...
        ML_j, bj  = apply_dirichlet_row_replace(M_jam, src_nodes, 1.0)
        ML_j, b2j = apply_dirichlet_row_replace(ML_j, dst_nodes, 0.0)
        bj += b2j
        ...
        plt.xlabel('x (m)'); plt.ylabel('y (m)')
        phi_png = f"{save_prefix}_{i}_phi.png"
        plt.tight_layout(); plt.savefig(phi_png, dpi=220)
        if not show:
            plt.close()
        print("Saved Phi field:", phi_png)

        greedy_nodes = greedy_path_on_nodes(phi, efrom_j, eto_j, src, dst, coords=coords)
        dijk_nodes, dijk_time = dijkstra_path(coords, efrom_j, eto_j, etime_jam, src, dst)
