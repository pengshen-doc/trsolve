...
def dijkstra_on_phi(coords, efrom, eto, phi, src, dst):
    """
    Run Dijkstra using edge weights derived from node potentials (avg of endpoints).
    """
    w = 0.5 * (phi[efrom] + phi[eto])
    #w = np.maximum(w, 0.0)
    rows = np.concatenate([efrom, eto])
    cols = np.concatenate([eto, efrom])
...
def case_hex_with_disks(save_prefix=None, show=False, seed=42):
    ...
    # Paths: Dijkstra on phi, and Dijkstra on travel times
    src = nearest_node(coords, src_c); dst = nearest_node(coords, dst_c)
    greedy_nodes, greedy_cost = dijkstra_on_phi(coords, efrom, eto, phi, src, dst)
    dijk_nodes, dijk_time = dijkstra_path(coords, efrom, eto, etime, src, dst)
    greedy_time = greedy_cost; ov = path_overlap(greedy_nodes, dijk_nodes)
...
def case_hex_with_jam_offsets(save_prefix="jam_hex", show=False, jam_factor=10000,
