...
def singum_tensor_per_node(coords, efrom, eto, K_edge, lattice, lp0, use_hex_iso=False):
    """
    ...
    For hex lattices, can optionally override with isotropic k_ij = (sqrt(3)/2)*K δ_ij.
    """
    N = coords.shape[0]
    kxx = np.zeros(N); kxy = np.zeros(N); kyy = np.zeros(N)
    V_s = 2*np.sqrt(3)*lp0**2 if lattice=='hex' else 4*lp0**2

    if lattice == 'hex' and use_hex_iso:
        Kbar = float(np.mean(K_edge))
        k_iso = 0.5 * np.sqrt(3.0) * Kbar
        kxx[:] = k_iso; kyy[:] = k_iso
        return kxx, kxy, kyy
...
def hex_isotropic_tensor(K_edge, N):
    """
    Isotropic hex mobility tensor using k_ij = (sqrt(3)/2) * K * δ_ij,
    where K is mean conductance (R = 1/K).
    """
    Kbar = float(np.mean(K_edge))
    k_iso = 0.5 * np.sqrt(3.0) * Kbar
    kxx = np.full(N, k_iso)
    ...
    while not reached_goal(cur) and steps < max_steps:
        ...
        if lower:
            ...
            if len(candidates) > 1:
                ...
                print("found a case here: ", len(candidates))
                #exit()
            else:
                nxt = candidates[0]
        else:
            ...
            print("steps = ",steps)
            # equal_sorted = sorted(equal, key=dist_to_goal)  # ascending (min to max)
            # nxt = equal_sorted[1]
...
def plateau_bfs_path(phi, efrom, eto, src, dst=None, goal_set=None, coords=None, max_steps=500000):
    """
    Plateau-aware BFS: flood equal-phi components, then expand to strictly lower
    neighbors. Never steps uphill; returns empty list if no goal is reachable.
    """
    from collections import defaultdict, deque
    nbrs = defaultdict(list)
    for u, v in zip(efrom, eto):
    ...
    while q and steps < max_steps:
        ...
        equal = [v for v in nbrs[u] if abs(phi[v] - phi[u]) <= tol]
        for v in equal:
            if v not in parent:
                parent[v] = u
                q.append(v)

        lower = [v for v in nbrs[u] if phi[v] < phi[u] - tol]
        lower.sort(key=lambda v: (phi[v], dist_to_goal(v)))
        for v in lower:
            if v not in parent:
                parent[v] = u
                q.append(v)
...
def continuous_gradient_path(phi, coords, src, dst, ngx=260, ngy=260, step_frac=0.10, max_steps=5000, initial_angle_deg=350.0):
    """
    ...
    xg = np.linspace(x_min, x_max, int(ngx))
    yg = np.linspace(y_min, y_max, int(ngy))
    X, Y = np.meshgrid(xg, yg)
    ...
    gradx_interp = RegularGridInterpolator((yg, xg), dPhidx, bounds_error=False, fill_value=0.0)
    grady_interp = RegularGridInterpolator((yg, xg), dPhidy, bounds_error=False, fill_value=0.0)

    pt = coords[int(src)].astype(float)
    goal = coords[int(dst)].astype(float)
    spacing = step_frac * min(xg[1]-xg[0], yg[1]-yg[0])
    ...
    if np.linalg.norm(goal - pt) > 1e-12:
        ...
        if np.isfinite(gx0) and np.isfinite(gy0) and gnrm>1e-6:
            ...
            step_dir = dir_goal
        #print("pt=",pt,"path=",path_xy,'gx,gy:',gx0,gy0,"gnrm:",gnrm,"step_dir=",step_dir)
        pt = pt + spacing * step_dir
    ...
    for index in range(int(max_steps)):
        ...
        if (index <=900):
            print("index = ", index, "location = ", pt," gradient = ", gn)
            #print("path_xy:",path_xy)
            #print("gn:",gn)
        else:
            break

        if np.linalg.norm(pt - goal) <= spacing:
    ...
    node_path = []
    for p in path_xy:
        idx = nearest_node(coords, p)
        if not node_path or node_path[-1] != idx:
            node_path.append(idx)

    return node_path, np.vstack(path_xy) if path_xy else np.zeros((0, 2))
...
def case_hex_with_disks(save_prefix=None, show=False, seed=42):
    ...
    kxx, kxy, kyy = singum_tensor_per_node(coords, efrom, eto, K_edge, lattice='hex', lp0=lp0_used, use_hex_iso=True)
    ...
    # Paths (continuous descent on phi; Dijkstra on times)
    src = nearest_node(coords, src_c); dst = nearest_node(coords, dst_c)
    greedy_nodes, greedy_xy = continuous_gradient_path(phi, coords, src, dst)
    ...
    plot_field_with_paths(X,Y,Phi,dPhidx,dPhidy, coords, src, dst,
                          greedy_coords=greedy_xy if len(greedy_nodes) else None,
...
def greedy_path_time(phi,efrom,eto,src,dst,time_map_jam,coords,choice):
    greedy_nodes, greedy_xy = continuous_gradient_path(phi, coords, src, dst)
    greedy_time = path_time(greedy_nodes, time_map_jam)
    return greedy_nodes, greedy_time, greedy_xy

#def case_hex_with_jam_offsets(save_prefix="jam_hex", show=False, jam_factor=10.0,offsets=(0.0, 1/3, 2/3, 1, 2.0)):
def case_hex_with_jam_offsets(save_prefix="jam_hex", show=False, jam_factor=1e10,offsets=[0.0]):
    """
    ...
    for i, off in enumerate(offsets, start=1):
        ...
        K_edge_jam = np.array([1.0 / max(t, 1e-12) for t in etime_jam], float)
        ...
        #dgreedy_time = path_time(greedy_nodes, time_map_jam)


        greedy_nodes,greedy_time,greedy_xy = greedy_path_time(phi,efrom,eto,src,dst,time_map_jam,coords=coords,choice=6)
        dijk_nodes, dijk_time = dijkstra_path(coords, efrom, eto, etime_jam, src, dst)
        kxx_iso, kxy_iso, kyy_iso, k_iso_scalar = hex_isotropic_tensor(K_edge_jam, coords.shape[0])
        ...
        if len(greedy_xy):
            plt.plot(greedy_xy[:,0], greedy_xy[:,1], 'b-', lw=0.2, label='Greedy')
        if dijk_nodes:
            plt.plot(coords[dijk_nodes,0], coords[dijk_nodes,1], 'm-', lw=0.2, label='Dijkstra')
        ...
        plt.quiver(Xs[mask], Ys[mask], U[mask], V[mask],
                   color='white', angles='xy', scale_units='xy',
                   scale=5000, width=0.0005, alpha=0.6, minlength=0.1)
        plt.plot(coords[src,0], coords[src,1], 'go', ms=7, label='Source')
        plt.plot(coords[dst,0], coords[dst,1], 'ro', ms=7, label='Destination')
        plt.gca().add_patch(Circle((center[0], center[1]), RADIUS, fill=False, ec='orange', lw=0.3, label='Jam'))
        plt.gca().set_aspect('equal', adjustable='box')
