...
def greedy_path_on_nodes(phi, efrom, eto, src, dst=None, goal_set=None, coords=None, choice=1,max_steps=500000):
    """
    ...
    while not reached_goal(cur) and steps < max_steps:
        ...
        lower = [v for v in nlist if phi[v] < phi[cur]]

        if(steps==74):
            print("at step cur=",cur, "phi[cur]=",phi[cur])
            for v in nlist:
                print("neighbor=",v, "phi=",phi[v]) 

        if lower:
            # print("lower=",lower[0],lower[1], "cur=",cur, "phi[cur]=",phi[cur])
            # print("neighbors' phi:", [phi[v] for v in nlist])
            # exit()
            ...
            if len(candidates) > 1:
                ...
                print("found a case here, steps=", len(candidates),steps)
                #exit()
            else:
                nxt = candidates[0]
        else:
            ...
            #max(equal,key=dist_to_goal)
            # print("steps = ",steps)
            # print("neighbors' phi:", [phi[v] for v in nlist])
            # exit()

            equal_sorted = sorted(equal, key=dist_to_goal)
            if steps==1:
                nxt = equal_sorted[1]
        ...
        if nxt in visited and nxt != path[-2] if len(path) >= 2 else False:
            print("exit here at step ", steps)
            # We're looping on a plateau: stop
            break

        # print the node coordinates for this step
        if (steps >= 60 and steps <= 100):
            if coords is not None:
                x, y = coords[nxt]
                print(f"step {steps}: node={nxt}, x={x:.6f}, y={y:.6f} phi={phi[nxt]:.6e}")
            else:
                print(f"step {steps}: node={nxt}")
...
def path_time_coords(path_coords, time_map, coords):
    """
    Compute path travel time when `path_coords` is an (M,2) array of (x,y)
    positions (output of `steep_path_fixed_step`). For each position we map
    to the nearest graph node and sum edge times from `time_map`.
    """
    if path_coords is None or len(path_coords) < 2:
        return np.nan
    # map each coordinate to nearest node index
    nodes = [nearest_node(coords, p) for p in np.asarray(path_coords)]
    ...
    if len(nodes_comp) < 2:
        return 0.0
    tt = 0.0
    for a, b in zip(nodes_comp[:-1], nodes_comp[1:]):
        t = time_map.get((a, b)) or time_map.get((b, a))
        if t is None:
            return np.nan
        tt += t
...
def plot_field_with_paths(X,Y,Phi,dPhidx,dPhidy, coords, src, dst,
    ...
    try:
        CS = plt.contour(X, Y, Phi, levels=20, colors='k', linewidths=0.8, alpha=0.55)
    except Exception:
        pass
...
def steep_path_fixed_step(phi, efrom, eto, src, dst=None, goal_set=None, coords=None,
                        step_len=0.002, max_steps=10000, grad_eps=1e-6, tol_goal=0.005):
    """
    Continuous steepest-descent path following -grad(phi) with fixed step length.
    - `phi`: 1D array of potential values at `coords`.
    - `coords`: (N,2) array of node coordinates corresponding to `phi`.
    - `src`: starting node index or 2-vector start coordinate.
    - `dst` / `goal_set`: optional goal node index or set of indices for stopping.
    - `step_len`: distance moved each step (meters in same units as `coords`).
    - `grad_eps`: finite-difference offset used to estimate gradient.

    Returns: ndarray of shape (M,2) with the sequence of (x,y) positions visited.
    """
    print("Starting continuous greedy path from src =",src)
    

    if coords is None:
        raise ValueError("coords must be provided for continuous path")
    coords = np.asarray(coords)
    phi = np.asarray(phi)

    # helper to evaluate phi at arbitrary point (linear interp, fallback nearest)
    def interp_phi(p):
        p = np.asarray(p, dtype=float).reshape(1, 2)
        val = griddata(coords, phi, p, method='linear')
        if val is None or np.isnan(val[0]):
            val = griddata(coords, phi, p, method='nearest')
        return float(val[0])

    # starting point (coordinate)
    if np.isscalar(src):
        cur = coords[int(src)].astype(float).copy()
    else:
        cur = np.asarray(src, float).reshape(2)

    # goal coordinates for stopping
    goal_coords = []
    if goal_set is not None:
        for g in goal_set:
            goal_coords.append(coords[int(g)])
    elif dst is not None:
        goal_coords.append(coords[int(dst)])
    if tol_goal is None:
        tol_goal = step_len * 1.0

    path = [cur.copy()]

    for step in range(max_steps):
        # finite-difference gradient estimate (central differences)
        dx = np.array([grad_eps, 0.0])
        dy = np.array([0.0, grad_eps])
        try:
            ph_xp = interp_phi(cur + dx)
            ph_xm = interp_phi(cur - dx)
            ph_yp = interp_phi(cur + dy)
            ph_ym = interp_phi(cur - dy)
        except Exception:
            print("Gradient eval failed at step ",step+1)
            break
        dphdx = (ph_xp - ph_xm) / (2.0 * grad_eps)
        dphdy = (ph_yp - ph_ym) / (2.0 * grad_eps)
        grad = np.array([dphdx, dphdy], float)
        gnorm = np.linalg.norm(grad)
        # If gradient is too small or invalid, step toward the goal (if provided).
        if not np.isfinite(gnorm) or gnorm <= 0.0:
            print("Gradient norm too small or non-finite at step ", step+1)
            if goal_coords:
                dels = np.asarray(goal_coords) - cur[None, :]            
                dists = np.linalg.norm(dels, axis=1)
                j = int(np.argmin(dists))
                dist_to_goal = float(dists[j])
                if dist_to_goal <= tol_goal:
                    break
                direction = dels[j] / max(dist_to_goal, 1e-12)
                nxt = cur + direction * float(step_len)
                print("gnorm:", gnorm, "direction:",direction,"nxt:",nxt,"cur:",cur)
                
                
            else:
                break
        else:
            # step in negative gradient (steepest descent)
            direction = -grad / gnorm
            nxt = cur + direction * float(step_len)

        # print the x,y coordinates of this step location

        xn, yn = float(nxt[0]), float(nxt[1])
        gx, gy = float(direction[0]), float(direction[1])
        print(f"step {step+1}: x={xn:.6f}, y={yn:.6f}, gx={gx:.6f}, gy={gy:.6f}")
        if step == 1:
            break
        ...
        if goal_coords:
            dists = np.linalg.norm(np.asarray(goal_coords) - cur[None, :], axis=1)
            print("Distances to goals:", dists)
            if np.any(dists <= tol_goal):
                break

        
    return np.vstack(path)

def steep_path(phi,efrom,eto,src,dst,time_map_jam,coords):
    # produce continuous coordinate path and compute its travel time
    greedy_coords = steep_path_fixed_step(phi, efrom, eto, src, dst=dst, goal_set=None, coords=coords,
                           step_len=0.005, max_steps=200, grad_eps=1e-6, tol_goal=None)
    greedy_time = path_time_coords(greedy_coords, time_map_jam, coords)
    return greedy_coords, greedy_time


#def case_hex_with_jam_offsets(save_prefix="jam_hex", show=False, jam_factor=10.0,offsets=(0.0, 1/3, 2/3, 1, 2.0)):
def case_hex_with_jam_offsets(save_prefix="jam_hex", show=False, jam_factor=10.0,offsets=[0.0]):
    """
    ...
    for i, off in enumerate(offsets, start=1):
        ...
        print(phi.shape)
        print(Phi.shape)
        print("src node:", src, " coord:", coords[src])
        print("dst node:", dst, " coord:", coords[dst])
        
        #greedy_nodes,greedy_time = greedy_path(phi,efrom,eto,src,dst,time_map_jam,coords=coords,choice=6)
        greedy_nodes,greedy_time = steep_path(phi,efrom,eto,src,dst,time_map_jam,coords=coords)
        ...
        if greedy_nodes is not None and len(greedy_nodes)>0:
            # `greedy_nodes` may be node indices or an (M,2) array of coordinates.
            g = np.asarray(greedy_nodes)
            if g.ndim == 2 and g.shape[1] == 2:
                plt.plot(g[:,0], g[:,1], 'c-', lw=1.0, label='Greedy')
            else:
                plt.plot(coords[g,0], coords[g,1], 'c-', lw=1.0, label='Greedy')
        if dijk_nodes:
            plt.plot(coords[dijk_nodes,0], coords[dijk_nodes,1], 'm-', lw=1.0, label='Dijkstra')
        # Overlay Phi contours on the same plot
        try:
        ...
        plt.quiver(Xs[mask], Ys[mask], U[mask], V[mask],
                   color='white', angles='xy', scale_units='xy',
                   scale=6000, width=0.0005, alpha=0.5, minlength=0.1)
        ...
        # print(f"[Jam case {i}] offset={off:.3f}×lp0 slowed={sum(slow_mask_arr)} edges "
        #       f"greedy={greedy_time:.5f}s -> {png_path}")
